...
```text
portfolio.rebalance_period_days = 3
```

每日仍计算 OTO 收益。调仓日使用 `F(t-1)` 重新十分组并更新持仓；非调仓日沿用上一调仓日的持仓，不重新按当日因子排序，手续费为 0。需要兼容旧版每日调仓时，将该配置设为 `1`。
...
`G0` 是方向标准化后的低因子值组，`G9` 是高因子值组。分组使用 `qcut` 和未固定种子的微噪声。手续费为 2023 年及以前 `0.0007`、2024 年起 `0.00035`，头组和等权基线均只在调仓日按成员变化收费。
...
每个因子保存摘要 JSON、每日 IC、IC 衰减、十分组日收益和累计收益、头组与基线超额收益、调仓日标记、时间对齐审计 CSV。运行时保存 `configs/evaluation.yaml` 的完整快照和哈希，Mongo 连接信息只从环境变量读取，禁止写入日志和产物。
