    ...
    <div className="space-y-5 animate-fade-in-up">
      ...
      <Card className="glass">
        ...
        <CardContent className="space-y-5">
          ...
          <div className="grid gap-3 border-y border-border py-4 text-sm md:grid-cols-4"><div><span className="text-muted-foreground">训练：</span>{config ? `${config.trainingStart} 至 ${config.trainingEnd}` : '--'}</div><div><span className="text-muted-foreground">验证：</span>{config ? `${config.validationStart} 至 ${config.validationEnd}` : '--'}</div><div><span className="text-muted-foreground">调仓：</span>{config ? `每 ${config.rebalancePeriodDays} 个交易日` : '--'}</div><div><span className="text-muted-foreground">时间对齐：</span>F(t-1) → open(t) → open(t+1)</div></div>
