...
type FactorClassification = { primary: string; secondary: string[]; reason: string };
const factorClassifications: Record<string, FactorClassification> = {
  df02445ad05db071: { primary: '动量', secondary: ['流动性'], reason: '5 日收益反转为核心，5/20 日量比作为调节项。' },
  '7ef9d13d6d24f999': { primary: '动量', secondary: ['流动性'], reason: '短期放量且收益为负时输出反转信号。' },
  '95c7a207894923e8': { primary: '动量', secondary: ['流动性'], reason: '反转强度和量比标准化后交互。' },
  '5985b341c52c44d7': { primary: '流动性', secondary: ['动量'], reason: '收益方向与成交量方向的一致性，刻画量价配合。' },
  d2384d0db48c1922: { primary: '流动性', secondary: ['动量'], reason: '收益与成交量变化的滚动相关。' },
  bc79a87d1993bfb3: { primary: '流动性', secondary: ['动量'], reason: '上涨放量与下跌放量天数差，刻画量价不对称。' },
  '38da793f6ba8733a': { primary: '动量', secondary: [], reason: '日内价格位置与 10 日趋势方向交互。' },
  '9b6534efa8484ba1': { primary: '动量', secondary: [], reason: '隔夜收益与日内收益的相对强弱。' },
  f7233eadb536931a: { primary: '动量', secondary: [], reason: '20 日价格位置与短期趋势方向交互。' },
  bce321c2a9884646: { primary: '流动性', secondary: ['动量'], reason: '20 日平均成交额排名调节 5 日收益。' },
  '9896bde1b0c46764': { primary: '流动性', secondary: ['动量'], reason: '中心化流动性排名与 5 日收益标准分交互。' },
  '9a590eb1ee51435d': { primary: '流动性', secondary: ['动量', '残差波动'], reason: '成交额排名、5 日收益和 20 日波动率共同交互。' },
  a1d342f500466b36: { primary: '流动性', secondary: ['动量'], reason: '成交额排名加权放量反转信号。' },
  befdae294e0b2b58: { primary: '残差波动', secondary: ['动量', '流动性'], reason: '用 20 日收益波动率调节放量反转。' },
  '0611579c82e4ecbb': { primary: '流动性', secondary: ['残差波动', '动量'], reason: '成交额排名、波动率排名和放量反转交互。' },
  '66e1a1c8dca44d7f': { primary: '动量', secondary: ['流动性'], reason: '短期跌势、缩量状态和日内收盘位置交互。' },
  '62d432d02f1155ca': { primary: '动量', secondary: ['流动性'], reason: '放量下跌、收盘低位和高开低走的延续状态。' },
  aec33f675c4df56f: { primary: '动量', secondary: [], reason: '短期下跌与隔夜/日内收益差结合。' },
  '3d57226fb0347783': { primary: '动量', secondary: ['流动性'], reason: '反转排名乘以量比排名。' },
  '86a3a4cc7f80c447': { primary: '动量', secondary: ['流动性'], reason: '反转强度乘以量比倒 U 型权重。' },
  f1c98ca31e3ceedb: { primary: '动量', secondary: ['流动性'], reason: '量比适中区间内输出反转排名。' },
};
const inferFactorClassification = (factor: Pick<Factor, 'factorExpression' | 'factorName'>): FactorClassification => {
  const text = `${factor.factorName || ''} ${factor.factorExpression || ''}`.toLowerCase();
  if (text.includes('std') || text.includes('volatility')) return { primary: '残差波动', secondary: text.includes('volume') ? ['流动性', '动量'] : ['动量'], reason: '表达式包含收益波动率或波动调节项。' };
  if (text.includes('volume') || text.includes('$amount')) return { primary: '流动性', secondary: text.includes('$return') ? ['动量'] : [], reason: '表达式包含成交量、成交额或量比信息。' };
  if (text.includes('$return') || text.includes('$open') || text.includes('$close')) return { primary: '动量', secondary: [], reason: '表达式主要由收益、价格位置或收益结构构造。' };
  return { primary: '未分类', secondary: [], reason: '当前规则未匹配到 Barra 风格类别。' };
};
const getFactorClassification = (factor: Factor | any): FactorClassification => {
  const id = factor.factorId || factor.factor_id;
  return factorClassifications[id] || inferFactorClassification({ factorName: factor.factorName || factor.factor_name || '', factorExpression: factor.factorExpression || factor.factor_expression || '' });
};
const categoryBadgeClass = (category: string) => category === '动量' ? 'border-blue-500/40 bg-blue-500/10 text-blue-700 dark:text-blue-300' : category === '流动性' ? 'border-emerald-500/40 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300' : category === '残差波动' ? 'border-amber-500/40 bg-amber-500/10 text-amber-700 dark:text-amber-300' : 'border-border text-muted-foreground';
const ClassificationBadges: React.FC<{ classification: FactorClassification; compact?: boolean }> = ({ classification, compact = false }) => <div className="flex flex-wrap gap-1" title={classification.reason}><Badge variant="outline" className={categoryBadgeClass(classification.primary)}>{compact ? classification.primary : `主类 ${classification.primary}`}</Badge>{classification.secondary.map((category) => <Badge key={category} variant="outline" className={categoryBadgeClass(category)}>{compact ? category : `辅类 ${category}`}</Badge>)}</div>;
const EXCESS_CHART_DOMAIN: [number, number] = [-0.5, 0.5];
const EXCESS_CHART_TICKS = [-0.5, -0.25, 0, 0.25, 0.5];
...
const ArtifactChart: React.FC<{ title: string; path?: string; kind: 'ic' | 'groups' | 'excess' | 'decay' }> = ({ title, path, kind }) => {
  ...
  const names: Record<string, string> = {
    G0: 'G0',
    G9: 'G9',
    long_short_half: '(G9-G0)/2 不含费',
    head_net_return: '头组累计 含费',
    benchmark_net_return: '基准累计 不含费',
    excess_return: '超额累计 头组含费-基准不含费',
    ic: 'IC',
    rank_ic: 'Rank IC',
  };
...
export const FactorLibraryPage: React.FC = () => {
  ...
  const visible = useMemo(() => factors.filter((factor) => {
    ...
    const classification = getFactorClassification(factor);
    return matchStatus && (!query || `${factor.factorName} ${factor.factorExpression} ${factor.factorDescription} ${classification.primary} ${classification.secondary.join(' ')}`.toLowerCase().includes(query));
  ...
  const selectedClassification = selected ? getFactorClassification(selected) : null;
  ...
  return <div className="space-y-5 animate-fade-in-up">
    ...
    <div className="overflow-x-auto border-y border-border"><table className="w-full min-w-[1040px] text-left text-sm"><thead className="text-xs text-muted-foreground"><tr><th className="px-3 py-3 font-medium">因子</th><th className="px-3 py-3 font-medium">分类</th><th className="px-3 py-3 font-medium">状态</th><th className="px-3 py-3 font-medium">方向</th><th className="px-3 py-3 font-medium">IC</th><th className="px-3 py-3 font-medium">ICIR</th><th className="px-3 py-3 font-medium">收益差（不含费）</th><th className="px-3 py-3 font-medium">超额 Sharpe（头组含费）</th></tr></thead><tbody className="divide-y divide-border">{visible.map((factor) => { const classification = getFactorClassification(factor); return <tr key={factor.factorId} onClick={() => select(factor)} className="cursor-pointer hover:bg-secondary/30"><td className="max-w-sm px-3 py-3"><div className="truncate font-medium" title={factor.factorName}>{factor.factorName}</div><div className="mt-1 truncate font-mono text-xs text-muted-foreground">{factor.factorExpression}</div></td><td className="min-w-36 px-3 py-3"><ClassificationBadges classification={classification} compact /></td><td className="px-3 py-3"><Badge variant="outline">{labels[statusOf(factor)] || statusOf(factor)}</Badge></td><td className="px-3 py-3 font-mono">{factor.directionMultiplier === -1 ? '-1' : factor.directionMultiplier === 1 ? '+1' : '--'}</td><td className="px-3 py-3 font-mono">{number(factor.trainingMetrics?.ic ?? factor.ic, 4)}</td><td className="px-3 py-3 font-mono">{number(factor.trainingMetrics?.icir ?? factor.icir)}</td><td className="px-3 py-3 font-mono">{number(factor.trainingMetrics?.long_short_spread)}</td><td className="px-3 py-3 font-mono">{number(factor.trainingMetrics?.excess_sharpe)}</td></tr>; })}</tbody></table>{!visible.length && <div className="py-14 text-center text-sm text-muted-foreground">没有符合条件的因子</div>}</div>

    {selected && <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setSelected(null)}><div className="max-h-[92vh] w-full max-w-5xl overflow-y-auto rounded-md border border-border bg-background shadow-2xl" onClick={(event) => event.stopPropagation()}><div className="sticky top-0 z-10 flex items-start justify-between border-b border-border bg-background px-5 py-4"><div className="min-w-0"><h2 className="break-words text-xl font-semibold">{selected.factorName || selected.factor_name}</h2><div className="mt-2 flex flex-wrap gap-2"><Badge variant="outline">{labels[evaluation.status || selected.evaluationStatus || 'not_evaluated']}</Badge><Badge variant="outline">方向 {evaluation.direction_multiplier === -1 ? '-1' : '+1'}</Badge><Badge variant="outline"><ShieldCheck className="mr-1 h-3 w-3" />2026 {evaluation.oos_status || selected.oosStatus || 'sealed'}</Badge>{selectedClassification && <ClassificationBadges classification={selectedClassification} />}</div></div><Button variant="ghost" onClick={() => setSelected(null)} title="关闭"><X className="h-4 w-4" /></Button></div>
      <div className="space-y-6 p-5">
        <section><h3 className="mb-3 text-sm font-medium">四项训练门槛</h3><div className="grid grid-cols-2 gap-4 md:grid-cols-4"><Metric label="|IC|" value={gates.ic?.value ?? training.ic_abs} passed={gates.ic?.passed} /><Metric label="ICIR" value={gates.icir?.value ?? training.icir} passed={gates.icir?.passed} /><Metric label="G9-G0 算术收益差（不含费）" value={gates.long_short_spread?.value ?? training.long_short_spread} passed={gates.long_short_spread?.passed} /><Metric label="头组超额 Sharpe（头组含费）" value={gates.excess_sharpe?.value ?? training.excess_sharpe} passed={gates.excess_sharpe?.passed} /></div></section>
        ...
        <ArtifactChart title="十分组累计收益（G0/G9 不含费，蓝线为 (G9-G0)/2 不含费）" path={artifacts.training_group_cumulative} kind="groups" /><ArtifactChart title="训练期每日 IC" path={artifacts.training_daily_ic} kind="ic" /><ArtifactChart title="IC 衰减与半衰期" path={artifacts.ic_decay} kind="decay" /><ArtifactChart title="头组（含费）、等权基准（不含费）与超额累计收益" path={artifacts.training_excess_returns} kind="excess" />
